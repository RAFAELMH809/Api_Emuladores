...
class MqttGateway {
  ...
  private async handleIncomingMessage(topic: string, payloadRaw: string): Promise<void> {
    const emulatorId = this.extractEmulatorId(topic);
    if (!emulatorId || !this.telemetryHandler) {
      return;
    }

    try {
      const payload = JSON.parse(payloadRaw) as Record<string, unknown>;
      await this.telemetryHandler({ topic, emulatorId, payload });
    } catch (error: unknown) {
      logger.error("MQTT message handling error", error);
    }
  }

  private extractEmulatorId(topic: string): string | null {
    const parts = topic.split("/");
    if (parts.length < 3 || parts[0] !== "safeair" || parts[2] !== "telemetry") {
      return null;
    }
